...
extension kip_dashboard {
    ...
    func configureApi(_ app: HBApplication, jwtAuthenticator: JWTAuthenticator) {
        ...
        app.router
            .group("api")
            // .add(middleware: jwtAuthenticator)
            .get("/orderSalesTrend.json") { _ -> HBResponse in
        ...
        app.router.group("api")
            .add(middleware: jwtAuthenticator)
            .get("/locations.json") { _ -> HBResponse in
        ...
        app.router
            .group("api")
            .add(middleware: jwtAuthenticator)
            .get("allLocationsOrdersByDay.json") { _ -> HBResponse in
        ...
        app.router.group("api")
        ...
        #if canImport(AppKit)
            app.router.get("/items.csv") { _ -> HBResponse in
            ...
            app.router.get("/itemsTest.csv") { _ -> HBResponse in
